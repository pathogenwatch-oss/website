...
module.exports = function ({ genomeId, fileId, organismId, speciesId, genusId, clientId, tasks }) {
  const name = organismId ? queues.tasks : queues.specieator;
  const promises = tasks.map(({ task, version, retries = defaultRetries }) =>
    enqueue(
      name, { genomeId, fileId, organismId, speciesId, genusId, clientId, task, version, retries }
    )
  );
